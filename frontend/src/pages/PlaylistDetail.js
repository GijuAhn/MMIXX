...
const PlaylistDetail = () => {
  ...
  const { audioElement, nowMusic, queue, isPlaying, setIsPlaying, createNowPlaylist, createNowMusic } = usePlayControl(playlistSeq);
  ...
  useEffect(() => {
    ...
    getPlaylistInfo(playlistSeq, atomUser.userSeq)
      ...
      .then((res) => {
        getUser(res).then((res) => setPlaylistUserInfo(res.data));
...
const MoreIconDiv = ({ playlistMusic, playlistSeq }) => {
  ...
  return (
    <MoreDiv ref={wrapperRef}>
      <SelectSection>
        <MoreVertIcon fontSize='small' onClick={handleToggle} />
        {isOpen && (
          <CustomSelect>
            <ul>
              <li
                onClick={() =>
                  navigate(`/playlist/select/update`, {
                    state: {
                      playlistSeq: `${playlistSeq}`,
              ...
              <li onClick={() => navigate(`/playlist/edit/${playlistSeq}`)}>수정하기</li>
              <li onClick={confirmDelete}>삭제하기</li>
            </ul>
          </CustomSelect>
        )}
      </SelectSection>
